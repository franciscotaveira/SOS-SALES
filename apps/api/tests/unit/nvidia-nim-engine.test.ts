...
describe('NVIDIA NIM commercial model contract', () => {
  ...
  it('uses a currently available Nemotron endpoint for the fast tier', async () => {
    expect(NVIDIA_MODEL_TIERS.FAST).toBe('nvidia/nemotron-3-super-120b-a12b');
    expect(NVIDIA_MODEL_TIERS.NEMOTRON).toBe('nvidia/nemotron-3-super-120b-a12b');
  ...
  it('disables reasoning traces for customer-facing Nemotron responses', async () => {
    ...
    expect(payload.model).toBe('nvidia/nemotron-3-super-120b-a12b');
