...
ssh "${VPS_ALIAS}" \
  "test ! -e '${REMOTE_RELEASE}' && test ! -e '${REMOTE_STAGING}' && mkdir -p '${REMOTE_STAGING}/web/dist' '${REMOTE_STAGING}/api/dist' '${REMOTE_STAGING}/api/node_modules' '${REMOTE_STAGING}/certs'"
...
rsync -avz --delete "${REPO_ROOT}/apps/api/node_modules/" "${VPS_ALIAS}:${REMOTE_STAGING}/api/node_modules/"
...
ssh "${VPS_ALIAS}" \
  "cd '${REMOTE_ROOT}' && test -s .env.production && grep -Eq '^META_VERIFY_TOKEN=[[:space:]]*[^[:space:]]' .env.production && grep -Eq '^META_APP_SECRET=[[:space:]]*[^[:space:]]' .env.production && grep -Eq '^WAHA_API_KEY=[[:space:]]*[^[:space:]]' .env.production && grep -Eq '^WAHA_WEBHOOK_SECRET_[A-Za-z0-9_]+=[[:space:]]*[^[:space:]]' .env.production && find '${REMOTE_STAGING}/api/node_modules' -mindepth 1 -maxdepth 1 -print -quit | grep -q . && chmod 0644 '${REMOTE_STAGING}/certs/supabase-ca.crt' && SOS_SALES_RELEASE_ROOT='${REMOTE_STAGING}' docker compose --env-file .env.production -f '${REMOTE_STAGING}/docker-compose.yml' config --quiet && mv '${REMOTE_STAGING}' '${REMOTE_RELEASE}'"
