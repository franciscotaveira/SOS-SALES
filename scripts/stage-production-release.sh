...
rsync -avz "${REPO_ROOT}/scripts/verify-production-schema-contract.mjs" "${VPS_ALIAS}:${REMOTE_STAGING}/scripts/verify-production-schema-contract.mjs"
...
ssh "${VPS_ALIAS}" \
  "cd '${REMOTE_ROOT}' && test -s .env.production && grep -Eq '^DATABASE_URL=[[:space:]]*[^[:space:]]' .env.production && grep -Eq '^SUPABASE_URL=https://[^[:space:]]+\\.supabase\\.co/?[[:space:]]*$' .env.production && grep -Eq '^META_VERIFY_TOKEN=[[:space:]]*[^[:space:]]' .env.production && grep -Eq '^META_APP_SECRET=[[:space:]]*[^[:space:]]' .env.production && grep -Eq '^WAHA_API_KEY=[[:space:]]*[^[:space:]]' .env.production && test -f '${REMOTE_STAGING}/api/package-lock.json' && candidate_node_image=\$(awk '/^  sos-sales-api:/ { in_api=1; next } in_api && /^  [^[:space:]]/ { exit } in_api && /^[[:space:]]+image:[[:space:]]*/ { sub(/^[[:space:]]+image:[[:space:]]*/, \"\"); print; exit }' '${REMOTE_STAGING}/docker-compose.yml') && test -n \"\${candidate_node_image}\" && docker run --rm -v '${REMOTE_STAGING}/api:/app' -w /app \"\${candidate_node_image}\" npm ci --omit=dev --ignore-scripts && find '${REMOTE_STAGING}/api/node_modules' -mindepth 1 -maxdepth 1 -print -quit | grep -q . && find '${REMOTE_STAGING}/api/supabase/migrations' -type f -name '*.sql' -print -quit | grep -q . && test -f '${REMOTE_STAGING}/api/supabase/config.toml' && test -f '${REMOTE_STAGING}/scripts/verify-production-schema.mjs' && test -f '${REMOTE_STAGING}/scripts/verify-production-schema-contract.mjs' && test -f '${REMOTE_STAGING}/scripts/PRODUCTION_MIGRATIONS_OPERATOR.md' && chmod 0644 '${REMOTE_STAGING}/certs/supabase-ca.crt' && SOS_SALES_RELEASE_ROOT='${REMOTE_STAGING}' docker compose --env-file .env.production -f '${REMOTE_STAGING}/docker-compose.yml' config --quiet && mv '${REMOTE_STAGING}' '${REMOTE_RELEASE}'"
