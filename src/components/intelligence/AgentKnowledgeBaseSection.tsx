...
export const AgentKnowledgeBaseSection: React.FC<AgentKnowledgeBaseSectionProps> = ({
  ...
  const handleFileUpload = async (files: FileList | null) => {
    ...
    if (!canManage) {
      setUploadError('Somente o proprietário do workspace pode editar a base de conhecimento.');
      return;
    }
    setUploadError(null);
    if (!onUploadDocument) {
      setUploadError('O upload precisa estar conectado ao backend deste workspace. Nenhum arquivo foi salvo.');
      return;
    }
    setIsUploading(true);
    ...
    for (const file of Array.from(files)) {
      ...
      if (!['txt', 'csv'].includes(fileType)) {
        setUploadError(`“${file.name}” não foi salvo: PDF, DOCX e XLSX ainda precisam de um extrator no backend. Envie TXT ou CSV para evitar dados falsos.`);
        continue;
      }
      ...
      try {
        ...
        if (!content.trim()) {
          setUploadError(`“${file.name}” não foi salvo porque está vazio.`);
          continue;
        }
        const saved = await onUploadDocument({
          name: file.name,
          fileType,
        ...
        if (saved) uploadedDocs.push(saved);
      } catch {
        setUploadError(`Falha ao persistir “${file.name}”. Verifique o backend e tente novamente.`);
      }
    }
    if (uploadedDocs.length > 0) {
      const updated = [...uploadedDocs, ...documents];
  ...
  const handleCreateRuleDoc = (e: React.FormEvent) => {
    e.preventDefault();
    if (!canManage) {
      setUploadError('Somente o proprietário do workspace pode criar regras.');
      return;
    }
  ...
  const handleDeleteDoc = async (id: string) => {
    if (!canManage) {
      setUploadError('Somente o proprietário do workspace pode remover documentos.');
      return;
    }
    if (onDeleteDocument && !(await onDeleteDocument(id))) {
      setUploadError('Não foi possível remover o documento no backend.');
      return;
    }
    ...
    switch (status) {
      ...
      case 'pending':
        return {
          ...
          className: 'bg-[var(--sos-warning-subtle)] text-[var(--sos-warning)] border-[var(--sos-warning)]/30',
  ...
  return (
    <div className="space-y-4 animate-in fade-in duration-200">
      {/* Header Banner */}
      {uploadError && (
        <div role="alert" className="rounded-lg border border-rose-300 bg-rose-50 px-3 py-2 text-[10px] text-rose-900">
          {uploadError}
        </div>
      )}
      <div className="bg-[var(--sos-surface)] border-[var(--sos-border)] rounded-xl p-4 shadow-2xs flex flex-col md:flex-row md:items-center justify-between gap-3">
        <div className="space-y-1">
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-[8.5px] bg-[var(--sos-ai)]/20 text-[var(--sos-ai)] font-bold px-1.5 py-0.5 rounded-full border border-[var(--sos-ai)]/30 flex items-center gap-1">
              <Database className="w-2.5 h-2.5" /> Conteúdo disponível para o agente
            </span>
          </div>
          <p className="text-[9.5px] text-[var(--sos-muted)]">
            Envie TXT ou CSV com preços, FAQs e regras comerciais. O conteúdo é persistido no backend e só fica disponível ao agente depois de salvo.
          </p>
        </div>
        ...
        <div className="flex items-center gap-2.5 shrink-0 bg-[var(--sos-background)] px-3 py-1.5 rounded-lg border border-[var(--sos-border)]">
          ...
          <div className="text-center">
            <span className="text-[8.5px] text-[var(--sos-muted)] block font-semibold">Chunks calculados</span>
            <span className="text-sm font-bold text-[var(--sos-ai)]">{totalChunks}</span>
          </div>
          <div className="h-5 w-px bg-[var(--sos-border)]" />
          <div className="text-center">
            <span className="text-[8.5px] text-[var(--sos-muted)] block font-semibold">Tokens estimados</span>
      ...
      <div
        ...
        className={`border-2 border-dashed rounded-lg p-4 text-center cursor-pointer transition-all flex flex-col items-center justify-center gap-1.5 group ${
          ...
          : canManage
            ...
            : 'bg-slate-50 border-slate-200 text-slate-500 shadow-2xs cursor-not-allowed'
        }`}
      >
        <input
          type="file"
        ...
        <div className="space-y-0.5">
          <p className="text-[9.5px] font-bold text-[var(--sos-ink)] font-heading">
            {isUploading
              ? 'Lendo conteúdo e persistindo no backend...'
              : 'Clique ou arraste arquivos para treinar o Agente de IA deste cliente'}
          </p>
          <p className="text-[8.5px] text-[var(--sos-muted)]">
            Disponível nesta versão: <span className="font-semibold text-[var(--sos-ink)]">CSV e TXT</span>. PDF, DOCX e XLSX aguardam extrator no backend.
          </p>
        </div>
        ...
        <div className="flex items-center gap-1.5 pt-1">
          ...
          <button
            ...
            disabled={!canManage}
            className="text-[8.5px] text-[var(--sos-ai)] font-bold hover:underline flex items-center gap-0.5 disabled:cursor-not-allowed disabled:opacity-50"
          >
            ...
            >
              <div className="flex items-start gap-2.5 min-w-0">
                ...
                <div className="space-y-0.5 min-w-0">
                  <div className="flex items-center gap-1.5 flex-wrap">
                    ...
                    <span className={`text-[8px] font-bold px-1.5 py-0.2 rounded border flex items-center gap-1 ${getDocumentStatus(doc.status).className}`}>
                      {getDocumentStatus(doc.status).icon} {getDocumentStatus(doc.status).label}
                    </span>
              ...
              <div className="flex items-center gap-1 shrink-0 self-end sm:self-center">
                <button
                  ...
                  disabled={!canManage}
                  className={`px-2 py-1 rounded-lg text-[8px] font-semibold flex items-center gap-0.5 transition-colors disabled:cursor-not-allowed disabled:opacity-50 ${
                    doc.isPrioritizedFact
                ...
                >
                  <Eye className="w-3 h-3" /> Ver conteúdo
                </button>
                <button
                  onClick={() => handleDeleteDoc(doc.id)}
                  disabled={!canManage}
                  className="p-1 rounded-lg text-[var(--sos-muted)] hover:text-[var(--sos-danger)] hover:bg-[var(--sos-danger-subtle)] transition-colors disabled:cursor-not-allowed disabled:opacity-50"
            ...
            <div className="flex items-center justify-between pb-2 border-b border-[var(--sos-border)]">
              <div>
                <h3 className="text-xs font-bold text-[var(--sos-ink)] font-heading">
                  Auditoria do conteúdo persistido: {selectedDocForPreview.name}
                </h3>
                <p className="text-[8.5px] text-[var(--sos-muted)]">
                  {selectedDocForPreview.extractedChunksCount} chunks calculados • {selectedDocForPreview.tokenCount.toLocaleString()} tokens estimados
                </p>
              </div>
              <button
            ...
            <div className="flex-1 overflow-y-auto space-y-2.5 pr-1 text-[9px]">
              <div className="p-2.5 bg-[var(--sos-ai-subtle)] rounded-lg border border-[var(--sos-ai)]/30 text-[var(--sos-ink)] space-y-0.5">
                <span className="font-bold flex items-center gap-1 text-[9.5px]">
                  <Sparkles className="w-3 h-3 text-[var(--sos-ai)]" /> Resumo persistido
                </span>
              ...
              <div className="space-y-1.5">
                <h4 className="text-[8.5px] font-bold uppercase tracking-wider text-[var(--sos-muted)] font-heading">
                  Amostra do conteúdo persistido
                </h4>
                <div className="p-2.5 rounded-lg bg-[var(--sos-background)] border border-[var(--sos-border)] text-[var(--sos-ink)] space-y-0.5 font-mono text-[9px]">
                  <span className="font-bold text-[var(--sos-ink)]">Conteúdo salvo no workspace</span>
                  <p className="leading-relaxed whitespace-pre-wrap">
                    {selectedDocForPreview.rawContentSnippet?.trim() ||
                      selectedDocForPreview.summary?.trim() ||
                      'Nenhum conteúdo de pré-visualização persistido.'}
                  </p>
                </div>
              </div>
            </div>
