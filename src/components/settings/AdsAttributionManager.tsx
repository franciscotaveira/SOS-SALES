...
interface AdsAttributionManagerProps {
  workspace: Workspace;
}
...
export const AdsAttributionManager: React.FC<AdsAttributionManagerProps> = ({ workspace }) => {
  ...
  const handleSaveMeta = (e: React.FormEvent) => {
    e.preventDefault();
    alert('Configurações da Meta Ads & Conversions API (CAPI) salvas com sucesso no TX CRM Engine!');
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 bg-white p-6 rounded-2xl border border-slate-200/80 shadow-sm">
        <div>
          <h2 className="text-lg font-bold text-slate-900 flex items-center gap-2">
            ...
            Rastreamento & Atribuição de Anúncios (Meta Ads & Google Ads)
          </h2>
          <p className="text-sm text-slate-500 mt-0.5">
            Mapeie cliques, UTMs, Conversions API (CAPI) e promessas de anúncios para alimentar o Dossiê Vivo e a memória de gancho do lead.
          </p>
        </div>
        <div className="flex items-center gap-2">
          <span className="px-3 py-1 bg-emerald-50 text-emerald-700 border border-emerald-200 rounded-xl text-xs font-bold flex items-center gap-1.5">
            <CheckCircle2 className="w-4 h-4" /> TX CRM Attribution Sync Active
          </span>
        </div>
      </div>

      {/* Grid: Meta Ads & Google Ads Integration Cards */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-2xl p-6 border border-slate-200/80 shadow-sm space-y-4">
          <div className="flex items-center justify-between pb-3 border-b border-slate-100">
            <div className="flex items-center gap-2.5">
              ...
              <div>
                <h3 className="font-bold text-sm text-slate-900">Meta Ads & CAPI (WhatsApp Click-to-Chat)</h3>
                <p className="text-[11px] text-slate-500">Conexão oficial com Pixel & Conversions API</p>
              </div>
            </div>
          ...
          <form onSubmit={handleSaveMeta} className="space-y-3">
            ...
            <div>
              <label className="block text-[11px] font-bold text-slate-700 mb-1">Conversions API (CAPI) Access Token</label>
              <input
                type="password"
        ...
        <div className="bg-white rounded-2xl p-6 border border-slate-200/80 shadow-sm space-y-4">
          <div className="flex items-center justify-between pb-3 border-b border-slate-100">
            <div className="flex items-center gap-2.5">
              ...
              <div>
                ...
                <p className="text-[11px] text-slate-500">Rastreamento de campanhas de pesquisa e display</p>
              </div>
            </div>
            <label className="relative inline-flex items-center cursor-pointer">
              <input
          ...
          <form onSubmit={handleSaveGoogle} className="space-y-3">
            <div>
              <label className="block text-[11px] font-bold text-slate-700 mb-1">Google Ads Customer ID (MCC/Account)</label>
              <input
                type="text"
      ...
      {/* Campaign Hook & UTM Mapping Section */}
      <div className="bg-white rounded-2xl p-6 border border-slate-200/80 shadow-sm space-y-4">
        <div className="flex items-center justify-between">
          <div>
            <h3 className="text-sm font-bold text-slate-900 flex items-center gap-2">
              <Link2 className="w-4 h-4 text-[#00A884]" />
              Mapeamento de Campanhas, Promessas e Ganchos de Entrada
            </h3>
            <p className="text-xs text-slate-500 mt-0.5">
              Quando o lead chega do anúncio via WhatsApp com uma UTM específica, a IA já conhece a promessa exata feita no criativo.
            </p>
          </div>
          <button
          ...
          >
            <Plus className="w-4 h-4" /> Vincular Campanha
          </button>
        </div>
              ...
              <div>
                <div className="flex items-center justify-between mb-1.5">
                  ...
                  <span className="text-[11px] font-mono text-emerald-700 font-bold bg-emerald-50 px-2 py-0.5 rounded">
                    {camp.activeLeadsCount} leads ativos
                  </span>
                </div>
                ...
                <div className="mt-2.5 p-2.5 bg-white rounded-lg border border-slate-200 text-[11px] text-slate-700">
                  <span className="font-bold text-slate-950 block text-[10px] uppercase tracking-wider text-purple-700 mb-0.5">🎯 Promessa do Anúncio (Gancho):</span>
                  "{camp.hookPromise}"
                </div>
              </div>

              <div className="flex items-center justify-between pt-2 border-t border-slate-200/60 text-[11px]">
                <span className="text-slate-500 font-mono">Produto padrão: <b>{camp.defaultProduct}</b></span>
                <button
          ...
          <div className="bg-white w-full max-w-md rounded-2xl shadow-xl border border-slate-200 overflow-hidden animate-in fade-in zoom-in duration-200">
            ...
            <form onSubmit={handleAddCampaign} className="p-6 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-700 mb-1">Produto Associado</label>
                  <input
                    type="text"
                    value={newProduct}
              ...
              <div>
                ...
                <textarea
                  rows={3}
                  placeholder="Qual promessa o lead viu no anúncio? (Ex: Escova modelada com garantia de horário para casamento por R$59)"
                  value={newHook}
